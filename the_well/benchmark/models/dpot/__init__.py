...
class DPOTNet(nn.Module):
    def __init__(
        self, 
        dim_in: int,
        dim_out: int, # out channels, 1
        dset_metadata: WellMetadata = None,
        patch_size=16, 
        mixing_type = 'afno',
        in_timesteps = 1, 
        out_timesteps = 1, 
        ...
        img_size = dset_metadata.spatial_resolution if dset_metadata else (384, 128)
        ...
        if self.normalize:
            self.scale_feats_mu = nn.Linear(2 * self.in_channels, embed_dim)
            self.scale_feats_sigma = nn.Linear(2 * self.in_channels, embed_dim)
    ...
    ### in/out: B, X, Y, T, C
    def forward(self, x):
        x = rearrange(x, 'b x y c -> b x y 1 c')
        B, _, _, T, _ = x.shape
        if self.normalize:
